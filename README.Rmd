...
In the current release, `play()` uses hard-coded dealer logic, but simulation (`simulate_blackjack()`) allows for pluggable decision functions.

## Simulation Mode: Strategy Testing

Use `simulate_blackjack()` to test decision strategies against a standard dealer.
...
👉 Package Website (Documentation): [Reference tab](https://monasharp.github.io/game-package-wombats/reference/index.html) 
