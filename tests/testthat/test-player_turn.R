test_that("computer player hits and updates hand", {
  ...
  expect_true(length(vctrs::field(result$player_hands$Bot[[1]], "cards")) > 2)
...
test_that("computer player stands at score >= 17", {
  ...
  expect_equal(vctrs::field(updated_hand, "cards"), c("10", "7"))  # no change
})
...
test_that("computer player doubles down with valid score and coins", {
  ...
  updated_hand <- result$player_hands$Bot[[1]]
  updated_cards <- vctrs::field(updated_hand, "cards")
