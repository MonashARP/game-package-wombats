test_that("Computer player splits pair of 8s or Aces", {
  ...
  expect_length(res$Bot, 2)
  expect_true(all(length(vctrs::field(res$Bot[[1]], "cards")) == 2))
  expect_equal(vctrs::field(res$Bot[[1]], "cards")[1], "8")
  expect_equal(vctrs::field(res$Bot[[2]], "cards")[1], "8")
})
...
test_that("Computer player does NOT split pair of 9s", {
  ...
  expect_length(res$Bot, 1)  # Should not split
  expect_equal(vctrs::field(res$Bot[[1]], "cards"), c("9", "9"))
})
